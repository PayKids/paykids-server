...
public class ExpenseCategoryService {
    ...
    //이미 존재하는 카테고리인지 확인
    public Boolean isExistExpenseCategory(String token, String categoryName) {
        if (tokenService.expiredToken(token) == false){
            throw new MapperException(ErrorCode.TOKEN_EXPIRED_ERROR);
        }
    ...
    //소비 카테고리 저장 로직
    @Transactional
    public Boolean saveExpenseCategory(String token, String name) {
        if (tokenService.expiredToken(token) == false){
            throw new MapperException(ErrorCode.TOKEN_EXPIRED_ERROR);
        }
    ...
    //소비 카테고리 삭제, 기타인 경우 삭제하지 않음. 저장된 내역을 기타로 변경
    @Transactional
    public Boolean deleteExpenseCategory(String token, String name) {
        name = name.replaceAll("\\s", "");
        ...
        Optional<Category> changeCategory = categoryRepository.findByUserAndAllowanceTypeAndTitle(user, AllowanceType.EXPENSE, "기타");

        if (category.isPresent() && changeCategory.isPresent()) {
            List<AllowanceChart> allowanceChartList = allowanceChartRepository.findAllByUserAndAllowanceTypeAndCategory(user, AllowanceType.EXPENSE, category.get());
